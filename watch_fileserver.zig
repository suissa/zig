...
fn addWatchForDir(inotify_fd: i32, dir_path: []const u8, gpa: std.mem.Allocator, map: *WatchMap) !void {
    ...
    const copy = try gpa.dupe(u8, dir_path);
    try map.put(wd, copy);
}

fn addRecursive(inotify_fd: i32, root: []const u8, gpa: std.mem.Allocator, map: *WatchMap) !void {
    try addWatchForDir(inotify_fd, root, gpa, map);
    ...
    while (try it.next()) |entry| {
        ...
        try addRecursive(inotify_fd, child, gpa, map);
    }
}
...
pub fn main() !void {
    ...
    defer {
        ...
        if (leaked == .leak) std.log.err("memória vazando", .{});
    }
    ...
    var map = WatchMap.init(gpa);
    defer {
        var it = map.valueIterator();
        while (it.next()) |p| gpa.free(p.*);
        map.deinit();
    }

    try addRecursive(inotify_fd, root, gpa, &map);
    ...
    while (true) {
        ...
        while (off < n) {
            ...
            if ((ev.mask & posix.linux.IN.ISDIR) != 0 and (ev.mask & (posix.linux.IN.CREATE | posix.linux.IN.MOVED_TO)) != 0 and name_slice.len > 0) {
                const new_dir = try std.fs.path.join(gpa, &.{ wd_path, name_slice });
                defer gpa.free(new_dir);
                addWatchForDir(inotify_fd, new_dir, gpa, &map) catch {};
            }
